...
async function bootstrap() {
  // Runtime config first: apiBaseUrl, Cognito settings and thresholds all come
  // from /config.json so one image runs in every environment (design.md §9).
  ...
  setUnauthorizedHandler(() => {
    // Silent renew is handled by oidc-client-ts; a hard 401 after that means
    // the session is gone. Never leave a blank page (prd R1).
    if (!config.auth.devBypass) window.location.assign('/')
  })
