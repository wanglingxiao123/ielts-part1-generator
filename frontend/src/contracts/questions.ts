...
export interface AnswerKeyEntry {
  ...
  canonical: string
  /**
   * Conditional accepted variants (AR-004), not a per-answer quota. Empty is normal when no common mechanically equivalent form exists. Every entry must independently satisfy the same word/numeral budget and remain equivalent in context. Useful case, punctuation, spacing and hyphen variants may be explicit; exact trimmed duplicates are forbidden. Keep the list bounded and do not enumerate obscure typographic permutations or Cartesian products.
   */
  alternatives: string[]
  /**
