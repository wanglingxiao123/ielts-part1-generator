...
export interface Item {
  ...
  /**
   * v2 required (optional here only so v1 records validate; the v2 branch above requires it). Micro-category of the answer, used to check QR-027 (no micro-category tested by 3+ of the ten items). This is an INTERNAL closed taxonomy of this system, NOT a client-supplied enum: the client rule only asks for an answer category per item and offers location / price / service as examples. If the client ever supplies its own category list, this enum gives way to it. There is deliberately no 'other' fallback -- a catch-all bucket would make unrelated points collide and misfire the same-category count, and would give the model a 'when unsure, pick other' escape hatch that destroys the field's only value. A point that fits none of the 14 is an error, not a degradation: it belongs back in the material stage. Judge the nature of the answer, not the wording of the sentence. Where two values both look defensible the decision is an ORDERED procedure, not a weighing: (1) form first -- a name/date/time/span/currency/count settles it, and price is currency only; (2) a named occupation, vacancy, or role is job_title; (3) an artefact issued or shown is document, above the facility or service it governs; (4) contact is a route to a PERSON, so a reference code is not contact; (5) location is a position, above facility; (6) service vs facility is performed-vs-merely-present, NOT charged-vs-included; (7) preference needs alternatives named in the script, else requirement. First rule that fires ends it. Stated in generate-listening-part1/references/specification.md (authoritative) and machine-readable in feasibility-listening-part1/references/answer-category-decisions.json; prose boundaries alone let one run call an included breakfast a service and a named restaurant a facility while reading them as contradictory. Python checks the enum, the ten-item completeness and the counts; semantic accuracy is the audit agent's job.
   */
  answer_category?:
    | 'person_name'
    | 'contact'
