...
async def classify_question_revision(
    ...
    try:
        ...
        message = "\n\n".join([
            ...
            "Reviewer authority policy: an explicit transformation request is a requirement, not a "
            "suggestion. If the reviewer asks to change X to Y and the current artifact is X, route "
            "the work needed to produce Y even when X is valid, conventional, or arguably better. "
            "Do not return no_change merely because you prefer the current design or consider the "
            "requested design less appropriate. no_change is allowed only when the requested target "
            "state already exists, or when the comment is a factual defect claim that the artifacts "
            "prove false. A Form-to-Note, Note-to-Table, or equivalent explicit layout conversion "
            "whose target is not already present is replan_questions with replan_scope layout_only "
            "unless the reviewer also asks to replace information points.\n\n"
