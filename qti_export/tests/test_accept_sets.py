...
def rule_notes(spec):
    """Review lines other than the provenance note upstream's 7655c4a appends whenever QTI adds variants."""
    return [r for r in spec.review if "自动补充可接受写法" not in r]
...
def test_preprinted_currency_is_not_accepted_again_in_any_form():
    """R6 on top of the new parsing. Upstream's question rules (7655c4a) key only the bare value when
    the paper prints the currency, so both the word form and the symbol form are redundant."""
    spec = build("128", "price", form="numeric", prefix="£ ", suffix=" per night", evidence="£128 per night")
    assert spec.accept == ["128"]
    assert {"128 pounds", "£128", "£128.00"} <= set(spec.reject)
...
# ── upstream alternatives vs QTI filters ──────────────────────────────────────


def test_an_upstream_alternative_removed_by_a_qti_filter_is_called_out_for_review():
    """Upstream approved `£145`; the paper prints `£`, so R6 rejects it. That disagreement is the one
    line a reviewer must see, not something to infer from the reject set."""
    spec = accept_sets.build(
        {"number": 4, "answer_category": "price", "response_form": "numeric"},
        {"canonical": "145", "target": "145", "alternatives": ["£145", "145.00"]},
        TWO_WORDS, prefix="£", suffix=" per night", evidence="£145 per night",
    )
    # `145.00` carries no symbol, so it survives and is recorded as upstream-approved; `£145` does not.
    assert spec.accept == ["145", "145.00"]
    assert "£145" in spec.reject
    assert any("上游 alternatives 中以下写法被 QTI 规则剔除" in r and "£145" in r
               and "145.00" not in r for r in spec.review)
    assert spec.upstream_alternatives == ["145.00"]


def test_upstream_alternatives_that_survive_are_recorded_as_upstream_not_qti_added():
    spec = build("14 June", "date", prefix="Arrives: ")
    spec = accept_sets.build(
        {"number": 2, "answer_category": "date", "response_form": "phrase"},
        {"canonical": "14 June", "target": "14 June", "alternatives": ["14th June", "June 14"]},
        TWO_WORDS, prefix="Arrives: ",
    )
    assert spec.upstream_alternatives == ["14th June", "June 14"]
    assert "June 14th" in spec.qti_added_alternatives
    assert not any("被 QTI 规则剔除" in r for r in spec.review)
