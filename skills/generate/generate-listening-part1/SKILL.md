...
2. Build the ten-item blueprint.
   ...
   - Choose Form, Note, or Table from the information relationship, with no default or fallback.
     ...
     meaningful topic headings: requirements, preferences, procedures, facilities, advice, or
     arrangements discussed as points rather than entered as record fields. Table compares repeated
     entities against shared dimensions. A list is not automatically Form merely because each line
     can be given a label; ask whether those labels are genuine record fields or headings invented
     after the fact. Never choose a layout to satisfy a distribution quota.
   - A pure record field/value list is Form. For example, `Service stage / Arrangement` followed by
     `Driver visit`, `Collection slot`, `Student service charge`, `Included service`, and
     `Destination paperwork` is Form even if borders could make it look tabular. A Table needs at
     least two comparable data dimensions that organise like-with-like information. Two columns
     remain legal when they genuinely express those dimensions; never manufacture a third column.
     A planned Table must be a dense, meaningful information matrix: each ordinary row/column
     intersection will need either useful fixed information or an answer blank. Do not plan
