...
2. Build the ten-item blueprint.
   ...
   - Plan layout support: give each point an `item_form` (`form`, `table`, or `note` -- Part 1 delivers completion layouts only) and a **non-empty** `form_group`, so one group holds points sharing one natural candidate-visible layout. Each group's item numbers must be contiguous and its points must not be interleaved with another group's in the evidence order. Narrator windows control when evidence is heard, not where a continuous form/note/table must be split: do not split only at the midpoint cue, and do not merge genuinely different structures merely to reduce the group count. Record the same layouts in `completion_layout_coverage`.
