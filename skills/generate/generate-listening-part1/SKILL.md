...
2. Build the ten-item blueprint.
   ...
   - Plan layout support: give each point an `item_form` (`form`, `table`, or `note` -- Part 1 delivers completion layouts only) and a **non-empty** `form_group`, so one group holds 3+ points sharing a layout. Each group's item numbers must be contiguous, its points must not be interleaved with another group's in the evidence order, and no group may cross the narrator window boundary. Record the same layouts in `completion_layout_coverage`.
