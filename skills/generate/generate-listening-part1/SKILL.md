...
2. Build the ten-item blueprint.
   ...
   - Plan layout support: give each point an `item_form` (`form`, `table`, or `note` -- Part 1 delivers completion layouts only) and a **non-empty** `form_group`, so one group holds points sharing one natural candidate-visible layout. Each group's item numbers must be contiguous and its points must not be interleaved with another group's in the evidence order. Group count is content-driven and may be one, two, or three; there is no quota or preferred count. Record the same layouts in `completion_layout_coverage`.
   - Distinguish Form from Table by information structure, not appearance or column count. A pure
     field/value list, where every row names one unrelated record field and supplies its value, is
     a Form. For example, `Service stage / Arrangement` followed by `Driver visit`, `Collection
     slot`, `Student service charge`, `Included service`, and `Destination paperwork` is a Form
     even if borders could make it look tabular. A Table needs at least two comparable data
     dimensions that organise like-with-like information. Two columns remain legal when they
     genuinely express those dimensions; never manufacture a third column for appearance.
   - Write `blueprint_schema_version: 2` and give every item `response_form`, `answer_category` and `narrator_window_id`. Derive each from the item itself, not from a guess: `response_form` counts tokens of the actual `target` (a hyphenated compound is one word), `answer_category` picks from the 13 values with no catch-all, and `narrator_window_id` follows the split you declared. All three are recomputed and compared, so a wrong value is reported rather than accepted. See the reference's "Blueprint version 2" section for the values and the category boundaries.
