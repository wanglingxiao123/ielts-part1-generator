...
| 13 | **Pseudo-table or malformed matrix** | SC-015, QR-026 | Judge the printed axes and cells, not a mechanical column minimum. A real Table has at least two comparable data dimensions, meaningful column headings, and a rectangular `table_rows` matrix; every ordinary cell contains meaningful fixed information or a referenced question blank, and several blanks may share one row. A genuine two-column Table is legal. A pure field/value list, where every row names one unrelated record field and supplies its value, is Form: `Service stage / Arrangement` with `Driver visit`, `Collection slot`, charges, included service, and paperwork is the negative example. A sparse diagonal matrix whose row-specific facts each get their own column and whose other cells are padded with `—`, `-`, `N/A`, empty strings, or equivalent placeholders is also a pseudo-table: a dimension that applies to only one row is not a shared comparison axis. Filler headings (`Detail`, `Details`, `Notes`, `Information`, `Answer`), repeated row/cell wording, or unrelated field/value facts placed behind borders remain warning signs. Call it a pseudo-table when the grid communicates no relationship that a form or note would not express more naturally; never recommend inventing a third column or filler text. |
...
| 15 | **Forced split or layout mixing** | SC-015 | First test whether Q1-Q10 form one natural candidate-visible record. Groups should normally contain at least three questions. A one- or two-question group is a finding unless it represents a genuinely independent information structure that cannot naturally join either adjacent group. A boundary at the narrator midpoint is also a finding when the same record and layout continue on both sides; narrator windows constrain evidence, not page structure. Layouts alternating group by group with no corresponding change in the conversation are likewise variety for its own sake. A real change in record structure remains a valid reason to split. Say which boundary or layout lacks support and what continuous or replacement structure fits. |
| 16 | **Pseudo-form or suppressed Note** | SC-015, QR-026 | Form is for fields of one real record being completed. Note is for thematic explanatory points such as requirements, preferences, procedures, facilities, advice, reasons, or arrangements. A list does not become Form merely because every line can be assigned a short label. Flag Form when its labels are headings invented after the fact rather than fields the underlying application, booking, order, membership, or registration would naturally contain. Do not demand Note for variety; demand it only when the information relationship is thematic rather than record-based. |
