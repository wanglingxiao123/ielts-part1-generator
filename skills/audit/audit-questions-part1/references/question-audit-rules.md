...
| 13 | **Pseudo-table or malformed matrix** | SC-015, QR-026 | Judge the printed axes and cells, not a mechanical column minimum. A real Table has meaningful column headings and a rectangular `table_rows` matrix; fixed text and referenced question blanks occupy their actual cells, and several blanks may share one row. Filler headings (`Detail`, `Details`, `Notes`, `Information`, `Answer`), repeated row/cell wording, or unrelated field/value facts placed behind borders remain warning signs. Call it a pseudo-table when the grid communicates no relationship that a form or note would not express more naturally. |
