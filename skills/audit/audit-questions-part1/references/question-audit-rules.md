...
| 15 | **Required two-group structure** | SC-015 | Require exactly two groups with the same 4/6, 5/5, or 6/4 boundary announced by the narrator. The two layouts are independently selected and may be identical, so do not flag Form+Form, Note+Note, or Table+Table merely for repetition. Judge whether each selected layout truthfully represents its own information stage; report a pseudo-layout or a boundary that does not match the audio plan. |
